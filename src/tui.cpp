...
void Tui::startBenchmark(int onlyRow)
{
    ...
    // The benchmark starts the privileged helper through pkexec. Its prompt
    // may appear on the terminal, so the raw input reader is paused until
    // the first test starts, otherwise the password entry would be broken.
    if (AppSettings().getFlusingCacheState() && ::geteuid() != 0) {
        m_authPhase = true;
        if (m_keyNotifier)
            m_keyNotifier->setEnabled(false);
    }
    ...
    m_authPhase = false;
    if (m_keyNotifier)
        m_keyNotifier->setEnabled(m_screenActive);
...
void Tui::benchmarkStatusUpdate(const QString &name)
{
    ...
    // The first status is "Preparing...", the second one means the helper
    // prompt (if any) is over and the keys can be read again
    if (m_authPhase && m_runStatusCount >= 2) {
        m_authPhase = false;
        if (m_keyNotifier && m_screenActive)
            m_keyNotifier->setEnabled(true);
    }
